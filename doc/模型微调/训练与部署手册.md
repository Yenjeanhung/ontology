...
### 3.2 DPO 偏好对齐（第二阶段，SFT 完成后接续）

> 一句话：SFT 教会「输出什么」，DPO 用偏好对 `(chosen, rejected)` 修正 SFT 后
> 仍残留的系统性错误——错而自信、置信度失准、枚举/键名/格式漂移。
> 方法原理见《微调方法体系.md》§1；rejected 六类失败模式的设计依据见
> `训练数据集/dpo/gen_dpo_dataset.py` 文件头（全部映射线上 router_service.py 真实兜底分支）。

**偏好对数据**（本机生成，无需 GPU）：

```powershell
cd doc\模型微调\训练数据集\dpo
python gen_dpo_dataset.py 1000     # -> finetune/dpo/ontology_router_dpo.json（固定 seed 可复现）
```

**训练**（yaml 与数据同在 `finetune/dpo/`，在 SFT 合并模型上挂新 LoRA）：

```bash
# ① SFT 合并模型上传实例（DPO yaml 的 model_name_or_path 指向它）
scp -rP 39678 ./qwen06b-router-merged root@connect.nmb2.seetacloud.com:/root/autodl-tmp/models/
# ② 偏好对数据与 yaml 上传（dataset_info.json 已注册 ontology_router_dpo，无需再改）
scp -rP 39678 ./finetune/dpo/ontology_router_dpo.json root@connect.nmb2.seetacloud.com:/root/autodl-tmp/data/
scp -rP 39678 ./finetune/dpo/qwen06b_router_dpo.yaml root@connect.nmb2.seetacloud.com:/root/autodl-tmp/
# ③ 训练
llamafactory-cli train qwen06b_router_dpo.yaml
# ④ 合并导出 + 下载回本机：同 §3.1 命令，仅两处不同——
#    --adapter_name_or_path saves/qwen06b-router-dpo
#    --export_dir /root/autodl-tmp/models/qwen06b-router-dpo-merged
#    部署时用 DPO 版替换 SFT 版（Modelfile FROM / 直推路径不变即生效）；
#    回退 = 换回 qwen06b-router-merged，config INTENT_ROUTER_MODEL 改名即切换
```

**SFT vs DPO 关键差异**（面试高频）：

| 项 | SFT（§3.1） | DPO（本节） |
|---|---|---|
| 基座 | Qwen3-0.6B 原始权重 | SFT 合并模型（先教会说话，再教偏好） |
| stage / 数据格式 | sft / alpaca 三元组 | dpo / 偏好对（同一 prompt 的 chosen+rejected） |
| 学习率 | 2e-4 | 5e-6（小 1~2 个量级，过大会打崩 SFT 能力） |
| epoch | 3 | 2（DPO 过训典型症状：margin 拉满但生成退化） |
| 特有超参 | — | pref_beta: 0.1（KL 约束，对 SFT 参考模型的偏离惩罚） |
| 盯什么信号 | train loss 收敛 | rewards/margins 拉开且 chosen reward 不崩 |

**验收**：用 §6 同一 300 条留出集复测 DPO 版 vs SFT 版，重点看两类指标——
①边界难例准确率（定义问法误判 data、寒暄误判 kb 等 EDGE 样本）；
②置信度校准（正确标签的 conf 分布是否回归合理区间，直接决定线上门控质量）。
...
async def classify(query: str) -> dict:
    """返回 {"labels": [...], "confidence": float}；任何异常返回 None（调用方走全组合）。"""
...
- `chat` → 主大模型直答，不编排；
- 单标签 → 只勾选对应能力智能体进 Planner 输入（精简组合）；
- `None` / 低置信度 → 走现状全组合（兜底）；
...
| 模型 | 验收集 | 指标 | 达标线 |
|---|---|---|---|
| 意图路由 | 300 条人工标注 | 4 分类准确率 / 漏判率 | 准确率 ≥92%，data 类漏判 <5%（漏判走全组合兜底） |
| 意图路由 | 300 条人工标注 | 4 分类准确率 / 漏判率 | 准确率 ≥92%，data 类漏判 <5%（漏判走全组合兜底） |
