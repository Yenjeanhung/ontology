...
**先说结论：不需要全库重建——索引的最小管理单元是「文件」，文件内部再做「分片级」增量。**

**第一层：文件级隔离（架构保证）**。向量删除按分片 `embedding_id` 逐条删、图谱按 `file.id` 删文档子图、Chunk 行按 `file_id` 级联删——更新任一文件不影响同库其他文件；BM25 是内存索引，以向量库 `count()` 为指纹、分片数变化自动重建，零维护。所以"文档更新"从来不是全库重嵌，最多是单文件重建。

**第二层：分片级内容寻址增量（解决大文件小改动）**。朴素做法"删旧分片→整文件重新嵌入"的问题是 chunk_id 按位置编址（`{file_id}_{index}`）：中间改一段，后面所有分片 index 全部漂移，1000 片改 10 片也要全量重嵌——而嵌入是流水线最贵的一步。项目实现：`chunk_id` 改为**内容寻址**（`{file_id}_{内容hash前16位}`），分片内容 SHA-256 入库，重新处理时做 diff：

| 对比结果        | 动作                                                                                 |
| --------------- | ------------------------------------------------------------------------------------ |
| hash 命中       | **复用旧向量，零重嵌入**，仅刷新 index 等位置信息                              |
| 新增 / 内容变化 | 新嵌入 upsert（Chroma/Milvus 的 `add_documents` 均为 upsert 语义，同 id 幂等覆盖） |
| 旧有新无        | 删除向量 + 分片行                                                                    |

效果：1000 片只改 10 片 → 只嵌入 10 片，省 99% 嵌入开销；内容完全未变 + 分片策略未变 → 重复处理**零嵌入成本**（重新处理不再预清场，改由 diff 收敛）。另外上传时对流式计算整文件 SHA-256 做两件事：同 KB 重复内容检测（提示不阻塞）+ 处理幂等指纹。

**高频追问「中间插一段话，后面分片不就全漂移了，hash 怎么不变？」**：看切点跟着谁走。固定长度硬切的切点是绝对字符位置，插入确实让后面全漂移——该策略下只有插入点之前可复用，之后等于全量（方案边界，如实交代）。但主力策略（recursive/sentence/heading/semantic）的切点锚定在段落、句子、标题这些**内容自身的边界**上：插入的内容只被消化进插入点附近的 1~3 个分片（带 overlap 向后多波及 1 片），后面分片起点还是那些自然边界，**文本逐字节不变 → hash 不变**。即边界切分对插入是**局部敏感**而非全局敏感；hash 锁的是"这段文字"，不是"这段文字排第几"。内容寻址再补另一半：位置编址下 1 片拆 2 片会让后面所有 index 位移 → id 全变 → 旧向量成僵尸；内容寻址保证同一段文字不管排第几 id 都一样，复用才不串片。

**边界取舍**：图谱抽取保持文件级重建（实体归并复杂、文档子图整体替换，增量收益不匹配复杂度）；migration 之前的存量分片无 hash，不参与复用、按全量重建兜底；取消/删除文件仍走全清路径。

**记忆点**：文件级隔离是底线（更新不影响其他文件），分片级 content-hash 增量是优化（改哪嵌哪）；位置编址是增量化的死穴，内容寻址才让"分片漂移"命中复用；hash 不变的前提是**边界切分**——切点锚在段落/句子等内容边界而非字符位置，插入只影响附近几片；图谱按文件重建不跟增量。
...
- **挂在哪个节点**：指标本身阶段无关，算在哪个节点的结果集上就是哪的指标。实践口径——**Recall@K 评召回后**（召回负责别漏；重排候选来自召回，救不回漏检）、**Precision@K/MRR/NDCG 评重排后**的 top-N（重排只调序+截断，把相关的顶上去决定谁进 LLM）。即"召回管 Recall，重排管 Precision"。
...
RAGAS 是**端到端** RAG 评估框架，不是只评生成侧——四件套里 Faithfulness / Answer Relevancy 评 **answer**（生成侧），Context Precision / Context Recall 评 **contexts**（检索侧，即 recall/precision 的 LLM-as-judge 变体）。
...
## 你项目中ragas是怎么用的，说一下流程

我们项目的 RAGAS 评测是我自己搭的一套脚本，整体分三步：先建评测集，再跑链路采集，最后用 RAGAS 打分。

第一步，建评测集。一共 100 条，每条就是一个问题加一个标准答案。其中 98 条是正例，都是对着知识库原文出的题、逐条核对过，保证每个分片都覆盖到。这么做是因为第一版吃过亏：一开始只出了十几条，很多超出了语料范围，recall 只有 0.74，看起来像检索不行，其实是题本身出错了。所以后来改成每道题都在原文里有出处；量不够的部分，用合成脚本从分片反向生成 QA 对扩量，再人工筛掉不合格的。另外留了 10条负例，故意问知识库里没有的内容，比如行李赔偿、机票退改签，看系统会不会老实说找不到，而不是瞎编。

第二步，跑链路采集。拿着这 100个问题，逐条调线上同一套 RAG 链路——查询改写、向量加 BM25 混合检索、RRF 融合、重排、再生成——把每个问题检索到的上下文、生成的答案、耗时都存下来，连同当时的配置快照一起存成 JSONL，保证结果可以复现。

第三步，RAGAS 打分。把采集结果包成 RAGAS 的 SingleTurnSample，调 evaluate 接口，默认跑四个指标：忠实度、答案相关性、上下文精度、上下文召回。评估模型温度设成 0，最后输出每个指标的平均分，还有逐条的明细 CSV，哪条失败会标成 NaN，可以单独重算。

这里面有个我比较得意的设计：采集和打分是分开的。因为上下文和答案本来就是被测系统跑出来的，不是提前标注的，所以我先把采集结果存下来，之后换评估模型或者换指标，直接对存好的结果重新算分就行，不用重新跑一遍检索。另外还做了消融开关，一条命令能跑完整链路、纯向量 baseline、去掉 BM25 三组对比，这样每个组件贡献了多少就说得很清楚。

## 你项目中的ragas忠实度和答案相关度能达到多少

**先交代规模再报数**：知识库是自建的《航班运行处置建议》民航语料——1 份 docx 切 **10 个分片**全量向量化，同时绑定 4 个本体抽取实体关系建图；评测集 **100 条 = 98 条锚定原文的正例（核心题逐分片人工出、合成扩量后逐条核对，覆盖全部章节）+ 2 条知识库外负例（考察拒答行为）**，LLM-as-a-Judge 实测。

- **忠实度 0.85–0.95 区间（均值约 0.88）**：答案拆成原子声明后绝大多数条条有上下文支撑——prompt 硬性约束"只依据检索内容作答"+ 温度 0.1。图谱扩展会把相关实体的邻接分片带进来（top 12 chunks），模型偶尔综合出上下文"间接暗示但没明说"的结论，个别条目分被拉低，所以报区间不报单点。
- **答案相关度 0.85 左右**：直击问题、不回避。这个指标 0.9+ 反而少见——它是"从答案反推问题与原 query 的相似度"，答案写得越完整（带前提条件、例外情形）反向问题越发散，分数被摊薄；10条负例两链路都正确拒答（该指标按设计计 0 分），是刻意留的对照组。
- **GraphRAG 的真正价值在召回，且有一轮完整的踩坑—修复—复测**：图谱链路首版只有向量一路接图谱，查询词和图谱实体对不上就空检索（100 条里 30 多条），context_recall 只有 0.54；补 **BM25 三路 RRF 融合 + 空召回降阈值兜底改写**后 recall 升到 0.98、空检索归零、98 条正例 recall 无盲区，忠实度/相关度也随上下文质量同步恢复。一句话归因：**图谱和 BM25 提升"找得全"（检索侧），忠实度靠生成侧约束，两头分开归因**。

**被质疑"100 条评测集怎么保证质量"的应对**：核心题逐分片锚定原文人工出，覆盖全部章节；扩量用 ragas 的 `gen-testset` 从分片反向合成 QA 对，合成后逐条人工核对筛选——每条 reference 都核对过，**金标答案不来自被测系统本身**。且踩过真实的坑：首版评测集八成问题超出语料范围，recall 被拉到 0.74 虚低，锚定语料重写后恢复——**评测集超纲会把"检索没问题"错判成"检索差"**。工作顺序是先小集冒烟定位问题、再扩集验证稳定性。

**记忆点**：报数 0.85–0.95（忠实度）、0.85 左右（相关度）；规模口径 1 文档 10 分片 + 4 本体、100 题评测集（98 正例全覆盖 + 2 负例）；被问"评测集怎么来的"就答"核心题人工锚定 + gen-testset 合成扩量、逐条人工核对"；被问"GraphRAG 到底带来什么"就答"recall 0.54→0.98、空检索 30+→0，专治实体对不上的查询"。

## RAGAS 的 Context Precision/Recall 和检索侧 Precision@K/Recall@K 的区别
