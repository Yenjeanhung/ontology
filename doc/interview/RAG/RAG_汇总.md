...
**先交代规模再报数**：知识库是自建的《航班运行处置建议》民航语料——1 份 docx 切 **10 个分片**全量向量化，同时绑定 4 个本体抽取实体关系建图；评测集 **19 条 = 17 条锚定语料的正例（逐分片出题、覆盖全部章节）+ 2 条知识库外负例（考察拒答行为）**，LLM-as-a-Judge 实测。
...
- **答案相关度 0.85 左右**：直击问题、不回避。这个指标 0.9+ 反而少见——它是"从答案反推问题与原 query 的相似度"，答案写得越完整（带前提条件、例外情形）反向问题越发散，分数被摊薄；2 条负例两链路都正确拒答（该指标按设计计 0 分），是刻意留的对照组。
- **GraphRAG 的真正价值在召回，且有一轮完整的踩坑—修复—复测**：图谱链路首版只有向量一路接图谱，查询词和图谱实体对不上就空检索（19 条里 7 条），context_recall 只有 0.54；补 **BM25 三路 RRF 融合 + 空召回降阈值兜底改写**后 recall 升到 0.98、空检索归零、17 条正例 recall 无盲区，忠实度/相关度也随上下文质量同步恢复。一句话归因：**图谱和 BM25 提升"找得全"（检索侧），忠实度靠生成侧约束，两头分开归因**。

**被质疑"19 条太少"的应对**：语料本身就 10 个分片，17 条正例是逐分片锚定原文人工出的题——对这个量级的库是**全量覆盖而不是欠采样**，每条 reference 都逐字核对过（金标答案不来自被测系统本身）。且踩过真实的坑：首版评测集八成问题超出语料范围，recall 被拉到 0.74 虚低，锚定语料重写后恢复——**评测集超纲会把"检索没问题"错判成"检索差"**。扩量路径已备好：ragas 的 `gen-testset` 从分片合成 QA 对扩到几十上百条，先小集冒烟定位问题、再扩集验证稳定性，是这个语料量级下的正确顺序。

**记忆点**：报数 0.85–0.95（忠实度）、0.85 左右（相关度）；规模口径 1 文档 10 分片 + 4 本体、19 题评测集（17 正例全覆盖 + 2 负例）；被问"太少"就答"10 分片配 17 条锚定正例是全覆盖，合成扩量脚本已备好"；被问"GraphRAG 到底带来什么"就答"recall 0.54→0.98、空检索 7→0，专治实体对不上的查询"。
